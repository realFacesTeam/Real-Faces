var createWall = function (options){
  ...
  if(!options.window){
    ...
    var glass_geometry = new THREE.BoxGeometry( 21, 21, (width + 0.1) );
    ...
    var result = subtract_bsp.toMesh( new THREE.MeshLambertMaterial({ shading: THREE.SmoothShading, color:'black' }) );
...
var createWindowFrame = function (options){
  ...
  var cube_geometry = new THREE.BoxGeometry( 25, 25, 3 );
  var cube_mesh = new THREE.Mesh( cube_geometry );
  cube_mesh.position.x = x;
  cube_mesh.position.y = y;
  cube_mesh.position.z = z;
  var cube_bsp = new ThreeBSP( cube_mesh );
  var glass_geometry = new THREE.BoxGeometry( 20, 20, 3.1 );
  var glass_mesh = new THREE.Mesh( glass_geometry );
  glass_mesh.position.x = x;
  glass_mesh.position.y = y;
  glass_mesh.position.z = z;
...
var createWalls = function(){
  ...
  createWall({length:150, x:100, z:-25, rotated:true});
  createWall({length:200, x:0, z:-100});
